...
class Websocket:
    def __init__(self, query, cookie, league):
        ...
        socket = websocket.WebSocketApp(url, header=headers, on_message=self.message_handler, on_error=self.error_handler, on_close=self.error_handler, on_open=self.on_open, on_ping=self.on_ping)
        socket.run_forever()
